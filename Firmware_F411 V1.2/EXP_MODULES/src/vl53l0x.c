...
TaskHandle_t vl53TaskHandle = NULL;
...
void vl53l0xInit(void)
{
	...
	{
		reInit = true;
		vTaskResume(vl53TaskHandle);	/*恢复激光测距任务*/
	}
	else	/*首次接上光流模块*/
	{							
		xTaskCreate(vl53l0xTask, "VL5310X", 300, NULL, 6, &vl53TaskHandle);	/*创建激光测距模块任务*/
	}
...
static void vl53l0xSetParam(void)	/*设置vl53l0x 参数*/
{
	vl53l0xTest();		
	...
	while (1) 
	{
		...
		}else
		{
			...
			if(range_last >= 6550)	/*vl53 错误*/
			{
				...
				{
					count = 0;
					vTaskSuspend(vl53TaskHandle);	/*挂起激光测距任务*/					
				}				
			}else
			{
				count = 0;
...
bool vl53l0xReadRange(zRange_t* zrange)
{
	if (isInit) 
	{
