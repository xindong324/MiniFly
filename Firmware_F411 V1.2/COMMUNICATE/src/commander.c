...
{	
	...
	setpoint->velocity.z = -70.f - state->velocity.z;	/*降落速度 单位cm/s*/
	...
	if(commander.ctrlMode & 0x01)/*定高模式*/
	{
		...
		{	
			...
			if (initHigh == false)
			{
				...
				setFastAdjustPosParam(0, 1, 0, 80.f);	/*一键起飞高度80cm*/															
			}		
			...
			if (fabsf(climb) > 5.f)
			{
				...
				if(climb < -(CLIMB_RATE/3.f))	/*油门下拉过大*/
				{
					isMuchPullDown = true;
				}
			}
			...
			if(state->acc.z > 500.f && isMuchPullDown == true)	/*油门下拉过大，飞机触地停机*/
			{
				isMuchPullDown = false;
				commander.keyFlight = false;
				estRstAll();	/*复位估测*/
...
//			errorPosX = constrainf(errorPosX, -0.3f, 0.3f);	/*误差限幅 单位m*/
//			errorPosY = constrainf(errorPosY, -0.3f, 0.3f);	/*误差限幅 单位m*/
		}
	}
	else	/*手动模式*/
	{
