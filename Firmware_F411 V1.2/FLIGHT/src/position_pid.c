...
#define PIDVX_OUTPUT_LIMIT	15.0f	//ROLL限幅	(单位°)
#define PIDVY_OUTPUT_LIMIT	15.0f 	//PITCH限幅	(单位°)
#define PIDVZ_OUTPUT_LIMIT	(40000)	/*PID VZ限幅*/

#define PIDX_OUTPUT_LIMIT	2.0f	//X轴速度限幅(单位m/s)
#define PIDY_OUTPUT_LIMIT	2.0f	//Y轴速度限幅(单位m/s)
...
{	
	...
	// Roll and Pitch
	attitude->pitch  = pidUpdate(&pidVX, setpoint->velocity.x - state->velocity.x);
	attitude->roll = pidUpdate(&pidVY, setpoint->velocity.y - state->velocity.y);
	
	// Thrust
	float thrustRaw = pidUpdate(&pidVZ, setpoint->velocity.z - state->velocity.z);
	
//	*thrust = thrustRaw + THRUST_BASE;
